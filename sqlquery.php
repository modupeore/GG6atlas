...
libraryid, organism, tissue, chrom, start, stop, geneid, genename, coverage, tpm, fpkm, fpkmconfhigh, fpkmconflow, fpkmstatus
</td></tr></table>';
...
libraryid, organism, tissue, genename, readcount
</td></tr></table>';
...
libraryid, organism, tissue, chrom, position, refallele, altallele, variantclass, zygosity, dbsnpvariant, source, consequence, geneid, genename, transcript, feature, genetype, aachange, codonchange, quality, proteinposition
</td></tr></table>';
	}
?>
</td></tr></table>
...
<?php
  	if ( !empty($db_conn) && (!empty($_POST['order']) || (!empty($_POST['search']) && !empty($_POST['fastbit'])) )) { //make sure an options is selected
		...
		$output = "query_".$explodedate.".txt";
        	//about_display($result, $result2);
		$pquery = "perl jedimaster.pl -nosql '".$_SESSION[$table]['fastbit']."' -query '".$_SESSION[$table]['select']."' -o $output";
		//print $pquery;
		shell_exec($pquery);
		if (file_exists("OUTPUT/$output")){
			echo '<form action="" method="post">';
			echo '<p class="gened">Download the results below. ';
			$newbrowser = "results.php?file=OUTPUT/$output&name=query.txt";
...
<?php
  if ( !empty($db_conn) && (!empty($_POST['order']) || !empty($_POST['search']) || !empty($_POST['downloadfiles'])) ) { //make sure an options is selected
	echo '<div class="menu">Output</div><div class="xtra">';
	echo '<form action="" method="post">';
	if ( (preg_match("/^select/", $_SESSION[$table]['select'])) || (preg_match("/^show/", $_SESSION[$table]['select'])) || (preg_match("/^desc/", $_SESSION[$table]['select'])) ){ 
		...
		if ($num_results == 0) {
			...
			about_display($result, $result2);
			$pquery = "perl jedimaster.pl -query '".$_SESSION[$table]['select']."' -o $output";
			if (isset($_POST['downloadfiles']) && !empty($_SESSION[$table]['select']) ){ 
				shell_exec($pquery);
			print("<script>location.href='results.php?file=OUTPUT/$output&name=query.txt'</script>");
			}
		}
	} else  { print "Only DQL statements are accepted"; }
  } else {
	//print "no db_conn"; 
  }
