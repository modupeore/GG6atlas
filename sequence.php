...
?>
    ...
    <select name="column">
        <?php
			$i = 0;
			$all_rows = $db_conn->query("select library_id, mappingtool, annotationfile, diffexpresstool, varianttool, variantannotationtool from $table");
  ...
  if(!empty($db_conn) && (!empty($_POST['order']) || !empty($_POST['meta_data']))) { //make sure an options is selected
    ...
    if ($num_total_result == 0){ //Cross check if libraries selected are in the database
      ...
      if(!empty($_POST['meta_data']) && isset($_POST['downloadvalues'])) { //If download Metadata
        ...
        $output = "sequence_".$explodedate.".txt";
        $pquery = "perl jedimaster.pl -query 'select * from $table where $table.library_id in ($dataline)' -o $output";
		shell_exec($pquery);
        print("<script>location.href='results.php?file=OUTPUT/$output&name=sequenceinfo.txt'</script>");
      }
      elseif(!empty($_POST['meta_data']) && isset($_POST['downloadfpkm'])) { //If download fpkm
		...
		$query = "select species from bird_libraries where library_id in ($dataline) limit 1";
		$result = mysqli_query($db_conn,$query);
		$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
		$organism = $row['organism'];
        
		$pquery = "perl jedimaster.pl -genexp --species '$organism' --samples '$dataline' --output $output";
		shell_exec($pquery);
