...
<?PHP
	...
	$query = "select $table.library_id, $table.bird_id, $table.species, $table.tissue, $table.notes, $table.date ,$statustable1.genestatus as genestatus, $statustable1.countstatus as countstatus, $statustable2.status as variantstatus from $table left outer join $statustable1 on $table.library_id = $statustable1.library_id left outer join $statustable2 on $statustable2.library_id = $table.library_id where $table.species = 'gallus' ";
?>
...
<?php
    ...
    //if the sort option was used
		...
		$_SESSION[$table]['column'] = "library_id";
		if ($is_term) {
		    $query .= "AND ";
		}
...
?>
    ...
    <select name="column">
        <?php
			$i = 0;
			$all_rows = $db_conn->query("select $table.library_id, $table.bird_id, $table.species, $table.tissue, $table.notes, $table.date from $table");
    ...
    <span>View samples with gene expression information:</span><input type="checkbox" name="rnull" value="true"><br>
		<span>View samples with gene raw counts information:</span><input type="checkbox" name="cnull" value="true">
	<!-- <br><span>View samples with variant information:</span><input type="checkbox" name="vnull" value="true"> -->
  ...
  if(!empty($db_conn) && (!empty($_POST['order']) || !empty($_GET['libs']) || !empty($_POST['meta_data']))) { //make sure an options is selected
    ...
    if ($num_total_result == 0){ //Cross check if libraries selected are in the database
      ...
      if(!empty($_POST['meta_data']) && isset($_POST['downloadvalues'])) { //If download Metadata
        ...
        $output = "metadata_".$explodedate.".txt";
        $pquery = "perl jedimaster.pl -query \"select $table.library_id, $table.bird_id, $table.species, $table.tissue, $table.notes, $table.date from $table where $table.library_id in ($dataline)\" -o $output";
				shell_exec($pquery); 
        print("<script>location.href='results.php?file=OUTPUT/$output&name=metadata.txt'</script>");
      }
      elseif(!empty($_POST['meta_data']) && isset($_POST['downloadfpkm'])) { //If download fpkm
        ...
        $dataline = rtrim($dataline, ",");$newdataline = rtrim($newdataline, ",");
				$output = "fpkm_".$explodedate.".txt";
				$query = "select species from bird_libraries where library_id in ($newdataline) limit 1";
				$result = mysqli_query($db_conn,$query);
				$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
				$organism = $row['species'];
						
				$pquery = "perl jedimaster.pl -genexp --fpkm --species '$organism' --samples '$dataline' --output $output";
				shell_exec($pquery);
        print("<script>location.href='results.php?file=OUTPUT/$output&name=fpkm.txt'</script>");
      }
        ...
        $dataline = rtrim($dataline, ",");$newdataline = rtrim($newdataline, ",");
				$output = "tpm_".$explodedate.".txt";
				$query = "select species from bird_libraries where library_id in ($newdataline) limit 1";
				$result = mysqli_query($db_conn,$query);
				$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
				$organism = $row['species'];
						
				$pquery = "perl jedimaster.pl -genexp --tpm --species '$organism' --samples '$dataline' --output $output";
				shell_exec($pquery);
        print("<script>location.href='results.php?file=OUTPUT/$output&name=tpm.txt'</script>");
      }
